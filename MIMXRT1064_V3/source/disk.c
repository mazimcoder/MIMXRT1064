...
usb_msc_struct_t g_msc_access(void){
...
void USB_DeviceClockInit(void)
{
    ...
    {
        if(!CLOCK_EnableUsbhs0PhyPllClock(kCLOCK_Usbphy480M, 480000000U))
        	PRINTF("Error CLOCK_EnableUsbhs0PhyPllClock\r\n");
       if(!CLOCK_EnableUsbhs0Clock(kCLOCK_Usb480M, 480000000U))
    	   PRINTF("Error CLOCK_EnableUsbhs0Clock\r\n");
    }
    else
    {
        if(!CLOCK_EnableUsbhs1PhyPllClock(kCLOCK_Usbphy480M, 480000000U))
        	PRINTF("Error CLOCK_EnableUsbhs1PhyPllClock\r\n");
        if(!CLOCK_EnableUsbhs1Clock(kCLOCK_Usb480M, 480000000U))
        	PRINTF("Error CLOCK_EnableUsbhs1Clock\r\n");
    }
    usb_status_t error = kStatus_USB_Success;
    error = USB_EhciPhyInit(CONTROLLER_ID, BOARD_XTAL0_CLK_HZ, &phyConfig);
    if( kStatus_USB_Success!=error)
    	PRINTF("Error USB_EhciPhyInit %d\r\n",(uint8_t) error);
}
void USB_DeviceIsrEnable(void)
{
    uint8_t irqNumber;
    ...
    error = EnableIRQ((IRQn_Type)irqNumber);
    if(kStatus_Success != error)
    	PRINTF("Error EnableIRQ %d \r\n",(uint8_t)error);
}
#if USB_DEVICE_CONFIG_USE_TASK
void USB_DeviceTaskFn(void *deviceHandle)
{
...
void USB_DeviceApplicationInit(void)
{
    ...
    if (kStatus_USB_Success != error)
    {
        usb_echo("Error USB_DeviceMscDiskStorageInit %d \r\n",(uint8_t) error);
        return;
    }
#if (defined(USB_DEVICE_CONFIG_USE_TASK) && (USB_DEVICE_CONFIG_USE_TASK > 0)) && \
    (defined(USB_DEVICE_MSC_USE_WRITE_TASK) && (USB_DEVICE_MSC_USE_WRITE_TASK > 0))
    ...
    error = USB_DeviceClassInit(CONTROLLER_ID, &msc_config_list, &g_msc.deviceHandle);

    if (kStatus_USB_Success != error)
    {
        usb_echo("Error USB_DeviceClassInit %d \r\n", (uint8_t) error);
    }
    else
    {
        usb_echo("USB device SD Card Mass Storage\r\n");
    ...
    error = USB_DeviceRun(g_msc.deviceHandle);
    if(kStatus_USB_Success != error)
    	PRINTF("Error USB_DeviceRun %d \r\n",(uint8_t)error);
}
